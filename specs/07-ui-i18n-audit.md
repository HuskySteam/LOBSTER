...
## Decision: How We Should Add i18n To `@lobster-ai/ui`
...
- Export them from `@lobster-ai/ui` via `packages/ui/package.json` exports (e.g. `"./i18n/*": "./src/i18n/*.ts"`).
...
- `packages/app/`:
  ...
  - Add a tiny bridge provider in `packages/app/src/app.tsx` to feed `useLanguage()` into `@lobster-ai/ui`'s `I18nProvider`.
...
- `packages/enterprise/`:
  ...
  - Merge `@lobster-ai/ui` dictionaries and (optionally) enterprise-local dictionaries.
...
1. Completed (2026-01-20): Add `@lobster-ai/ui` i18n context (`packages/ui/src/context/i18n.tsx`) + export it.
