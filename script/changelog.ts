...
import { $ } from "bun"
import { createLobster } from "@lobster-ai/sdk/v2"
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/lobster packages/sdk packages/plugin sdks/vscode github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/lobster/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/lobster/")) areas.add("core")
...
async function summarizeCommit(opencode: Awaited<ReturnType<typeof createLobster>>, message: string): Promise<string> {
...
export async function generateChangelog(commits: Commit[], opencode: Awaited<ReturnType<typeof createLobster>>) {
...
export async function buildNotes(from: string, to: string) {
  ...
  const opencode = await createLobster({ port: 0 })
  const notes: string[] = []
