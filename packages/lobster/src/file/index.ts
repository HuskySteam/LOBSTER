...
export namespace File {
  ...
  export async function read(file: string): Promise<Content> {
    ...
    try {
      ...
      if (e?.code === "ENOENT") {
        // File doesn't exist yet (new file), allow if lexical check passed
        realPath = full
      } else {
        throw new Error(`Access denied: cannot resolve path`)
      }
    }
    if (realPath !== full && !Instance.containsPath(realPath)) {
  ...
  export async function list(dir?: string) {
    ...
    if (realResolved !== resolved && !Instance.containsPath(realResolved)) {
      throw new Error(`Access denied: symlink escapes project directory`)
    }
