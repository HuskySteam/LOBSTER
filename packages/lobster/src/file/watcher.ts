...
declare const LOBSTER_LIBC: string | undefined
...
export namespace FileWatcher {
  ...
  const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
    try {
      const binding = require(
        `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${LOBSTER_LIBC || "glibc"}` : ""}`,
      )
  ...
  export function init() {
    if (Flag.LOBSTER_EXPERIMENTAL_DISABLE_FILEWATCHER) {
      return
    }
    state()
  }
