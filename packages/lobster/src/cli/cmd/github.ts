...
const WORKFLOW_FILE = ".github/workflows/lobster.yml"
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://github.com/HuskySteam/LOBSTER",
              ].join("\n"),
            )
          }
...
jobs:
  lobster:
    if: |
      ...
      contains(github.event.comment.body, ' /lobster') ||
      startsWith(github.event.comment.body, '/lobster')
    ...
    steps:
      ...
      - name: Run lobster
        uses: HuskySteam/LOBSTER/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.opencode.ai" : "https://github.com/HuskySteam/LOBSTER"
      ...
      try {
        ...
        console.log("lobster session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/lobster,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to lobster...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("lobster-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `lobster/${type}-${hex}-${timestamp}`
        }
        return `lobster/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[lobster session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the lobster infrastructure after your response",
