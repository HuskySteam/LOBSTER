...
import { EOL } from "os"
import { createLobsterClient, type Message, type LobsterClient, type ToolPart } from "@lobster-ai/sdk/v2"
...
function task(info: ToolProps<typeof TaskTool>) {
  const agent = Locale.titlecase(info.input.subagent_type ?? "general")
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run lobster with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running lobster server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: LobsterClient) {
      if (args.continue) {
    ...
    async function share(sdk: LobsterClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.LOBSTER_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: LobsterClient) {
    ...
    if (args.attach) {
      const sdk = createLobsterClient({ baseUrl: args.attach })
