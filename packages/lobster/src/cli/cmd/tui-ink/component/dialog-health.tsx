      ...
      <Box marginTop={1} flexDirection="column" gap={1}>
        <Box flexDirection="column">
          ...
          {lobster.projectQuality && (
            <Text color={theme.textMuted}>Analyzed {formatTimeAgo(lobster.projectQuality.analyzed_at)}</Text>
          )}
          {!lobster.projectQuality && !lobster.analysisRunning && (
            <Text color={theme.textMuted}>Run /health to trigger AI analysis</Text>
          )}
          {lobster.analysisRunning && <Text color={theme.accent}>Analyzing project quality...</Text>}
        </Box>
        ...
        {recentSessions.length > 0 && (
          <Box flexDirection="column">
            ...
            {recentSessions.map((item) => (
              <Box key={`${item.time}:${item.title}`} gap={1}>
