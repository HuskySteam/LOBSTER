    ...
    <Box gap={1} marginTop={1} marginBottom={1}>
      {props.tabs.map((tab) => {
        ...
        return (
          <Box key={tab.id}>
            <Text
              color={active ? tokens.list.selectedText : tokens.text.muted}
              backgroundColor={active ? tokens.list.selectedBackground : undefined}
              bold={active}
            >
              {` ${tab.label}${suffix} `}
            </Text>
...
export function StatusBadge(props: { tone?: BadgeTone; label: string }) {
  ...
  return (
    <Text color={tokens.status[tone]}>{`[${props.label}]`}</Text>
  )
}
...
export function EmptyState(props: { title: string; detail?: string }) {
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {props.detail ? <Text color={tokens.text.muted} dimColor>{props.detail}</Text> : null}
    </Box>
  )
}
