...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall lobster and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g lobster-ai",
      pnpm: "pnpm uninstall -g lobster-ai",
      bun: "bun remove -g lobster-ai",
      yarn: "yarn global remove lobster-ai",
      brew: "brew uninstall lobster",
      choco: "choco uninstall lobster",
      scoop: "scoop uninstall lobster",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Lobster!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# lobster") || content.includes(".lobster/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".lobster/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".lobster/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".lobster"))
    ) {
      continue
    }
