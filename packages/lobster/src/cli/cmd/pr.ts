...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run lobster",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for lobster session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found lobster session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`lobster import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting lobster...")
        UI.println()

        // Launch lobster TUI with session ID if available
        const { spawn } = await import("child_process")
        const lobsterArgs = sessionId ? ["-s", sessionId] : []
        const lobsterProcess = spawn("lobster", lobsterArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          lobsterProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`lobster exited with code ${code}`))
          })
          lobsterProcess.on("error", reject)
        })
      },
    })
  },
