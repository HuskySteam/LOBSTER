...
import type { Event } from "@lobster-ai/sdk/v2"
import type { EventSource } from "./context/sdk"

declare global {
  const LOBSTER_WORKER_PATH: string
}
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start lobster tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start lobster in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    const workerPath = await iife(async () => {
      if (typeof LOBSTER_WORKER_PATH !== "undefined") return LOBSTER_WORKER_PATH
