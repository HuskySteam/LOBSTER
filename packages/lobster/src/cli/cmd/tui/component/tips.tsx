...
const TIPS = [
  "Switch between {highlight}coder{/highlight}, {highlight}reviewer{/highlight}, and {highlight}tester{/highlight} agents for iterative code improvement",
  "Monitor session cost and token usage in the {highlight}sidebar{/highlight} or {highlight}footer{/highlight}",
  "Configure cost budgets in {highlight}.lobster/memory/cost-budget.json{/highlight}",
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public link to your conversation at lobster.ai",
  ...
  "Create {highlight}lobster.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/lobster/lobster.json{/highlight} for global config",
  ...
  "LOBSTER auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.lobster/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.lobster/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "LOBSTER auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "LOBSTER uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.lobster/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.lobster/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent LOBSTER from reading sensitive files",
  "Use {highlight}lobster run{/highlight} for non-interactive scripting",
  "Use {highlight}lobster --continue{/highlight} to resume the last session",
  "Use {highlight}lobster run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}lobster serve{/highlight} for headless API access to LOBSTER",
  "Use {highlight}lobster run --attach{/highlight} to connect to a running server",
  "Run {highlight}lobster upgrade{/highlight} to update to the latest version",
  "Run {highlight}lobster auth list{/highlight} to see all configured providers",
  "Run {highlight}lobster agent create{/highlight} for guided agent creation",
  "Use {highlight}/lobster{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}lobster github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/lobster fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.lobster/themes/{/highlight} directory",
  ...
  "Run {highlight}lobster debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/lobster{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with LOBSTER Zen for curated, tested models",
