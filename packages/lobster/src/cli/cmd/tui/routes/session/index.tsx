...
// border imports removed — flat design, no bordered boxes
import { Spinner } from "@tui/component/spinner"
import { ActivityBar } from "@tui/component/activity-bar"
...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@lobster-ai/sdk/v2"
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  █  █  ${UI.Style.TEXT_DIM}lobster -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        `  ▀▀▀▀  `,
      ].join("\n"),
    )
  })
      ...
      <box flexDirection="row">
        <box flexGrow={1} paddingBottom={1} paddingTop={1} paddingLeft={2} paddingRight={2} gap={1}>
          <Show when={session()}>
            ...
            <ActivityBar sessionID={route.sessionID} />
            <box flexShrink={0}>
  ...
  return (
    <>
      <Show when={text()}>
        ...
        >
          <box flexDirection="row" onMouseUp={props.onMouseUp} gap={1}>
            <text fg={color()} flexShrink={0}><b>&#10095;</b></text>
            <box flexGrow={1}>
              <text fg={theme.text}>{text()?.text}</text>
            </box>
            <Show when={ctx.showTimestamps()}>
              <text fg={theme.textMuted} flexShrink={0}>{Locale.todayTimeOrDateTime(props.message.time.created)}</text>
            </Show>
          </box>
          <Show when={queued()}>
            <text>
          ...
          <Show when={files().length}>
            <box flexDirection="row" paddingTop={1} paddingLeft={2} gap={1} flexWrap="wrap">
              <For each={files()}>
                {(file) => {
                  const bg = createMemo(() => {
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={0} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.LOBSTER_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
  ...
  return (
    <text paddingLeft={2} fg={props.when ? theme.textMuted : theme.text}>
      <Show fallback={<>~ {props.fallback}</>} when={props.when}>
        <span style={{ fg: theme.accent, bold: true }}>{props.icon}</span> {props.children}
      </Show>
    </text>
  )
}
...
function InlineTool(props: {
  ...
  return (
    ...
    >
      <Switch>
        <Match when={isRunning() && props.complete}>
          <box paddingLeft={2}>
      ...
      <Show
        ...
        fallback={
          <text paddingLeft={2} fg={theme.text}>
            <b>{props.title.replace(/^# /, "")}</b>
          </text>
        }
      >
        <Spinner color={theme.text}>{props.title.replace(/^# /, "")}</Spinner>
      </Show>
      {props.children}
      <Show when={error()}>
        <text fg={theme.error}>{error()}</text>
