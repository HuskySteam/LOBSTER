...
export namespace SessionPrompt {
  ...
  export const loop = fn(Identifier.schema("session"), async (sessionID) => {
    ...
    while (true) {
      ...
      // Team inbox delivery: inject queued messages as synthetic user messages
      if (session.team) {
        const inboxMessages = await TeamManager.deliverInbox(
          session.team.teamName,
          session.team.agentName,
        )
        if (inboxMessages.length > 0) {
          const lastModel = await (async () => {
          ...
          for (const inboxMsg of inboxMessages) {
            const syntheticUserMsg: MessageV2.User = {
              ...
              agent: session.team.agentName,
              model: lastModel,
            }
            await Session.updateMessage(syntheticUserMsg)

            const label =
              inboxMsg.type === "broadcast"
                ? `[Team broadcast from ${inboxMsg.sender}]`
                : inboxMsg.type === "shutdown_request"
                  ? `[Shutdown request from ${inboxMsg.sender}] (requestId: ${inboxMsg.requestId})`
                  : `[Message from ${inboxMsg.sender}]`
    ...
    // Set team member to idle on loop exit
    if (session.team) {
      TeamManager.setMemberStatus(session.team.teamName, session.team.agentName, "idle").catch((err) => {
        log.warn("failed to set team member idle on loop exit", { error: err })
      })
    }
    ...
    throw new Error("Unexpected state: no assistant response found after session loop completed, possibly due to abort or empty message stream")
  })
