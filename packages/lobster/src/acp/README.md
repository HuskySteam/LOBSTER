...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for lobster.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal lobster sessions
...
lobster acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via lobster's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | lobster acp
```
...
ACP sessions map cleanly to lobster's internal session model:
