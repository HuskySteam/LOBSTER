...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/lobster"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "lobster")
      default:
        return "/etc/lobster"
    }
  }

  const managedConfigDir = process.env.LOBSTER_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  function mergeConfigConcatArrays(target: Info, source: Info): Info {
    ...
    if (target.plugin_marketplaces && source.plugin_marketplaces) {
      merged.plugin_marketplaces = Array.from(new Set([...target.plugin_marketplaces, ...source.plugin_marketplaces]))
    }
    return merged
  }
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): https://github.com/HuskySteam/LOBSTER
    // 1) Remote .well-known/lobster (org defaults)
    // 2) Global config (~/.config/lobster/lobster.json{,c})
    // 3) Custom config (LOBSTER_CONFIG)
    // 4) Project config (lobster.json{,c})
    // 5) .lobster directories (.lobster/agents/, .lobster/commands/, .lobster/plugins/, .lobster/lobster.json{,c})
    ...
    if (Flag.LOBSTER_CONFIG) {
      result = mergeConfigConcatArrays(result, await loadFile(Flag.LOBSTER_CONFIG))
      log.debug("loaded custom config", { path: Flag.LOBSTER_CONFIG })
    }
    ...
    result.plugin_marketplaces = result.plugin_marketplaces || []

    const directories = [
      Global.Path.config,
      // Only scan project .lobster/ directories when project discovery is enabled
      ...(!Flag.LOBSTER_DISABLE_PROJECT_CONFIG
        ...
        : []),
      // Always scan ~/.lobster/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".lobster"],
    ...
    // .lobster directory config overrides (project and global) config sources.
    if (Flag.LOBSTER_CONFIG_DIR) {
      directories.push(Flag.LOBSTER_CONFIG_DIR)
      log.debug("loading config from LOBSTER_CONFIG_DIR", { path: Flag.LOBSTER_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(directories)) {
      if (dir.endsWith(".lobster") || dir === Flag.LOBSTER_CONFIG_DIR) {
    ...
    if (Flag.LOBSTER_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LOBSTER_PERMISSION))
    }
    ...
    if (Flag.LOBSTER_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.LOBSTER_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@lobster-ai/plugin": targetVersion,
    }
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@lobster-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@lobster-ai/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@lobster-ai/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const patterns = ["/.lobster/command/", "/.lobster/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const patterns = ["/.lobster/agent/", "/.lobster/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  const CC_PLUGIN_GLOB = new Bun.Glob("{plugin,plugins}/*/.claude-plugin/plugin.json")
  async function loadPlugin(dir: string) {
    ...
    // Scan for Claude Code plugin directories
    for await (const item of CC_PLUGIN_GLOB.scan({
      absolute: true,
      followSymlinks: true,
      dot: true,
      cwd: dir,
    })) {
      // item is like /path/plugins/my-plugin/.claude-plugin/plugin.json
      // We want /path/plugins/my-plugin/
      const pluginDir = path.dirname(path.dirname(item))
   ...
   * - For github: specs: extracts the last path segment
   *
   * @example
   * getPluginName("file:///path/to/plugin/foo.js") // "foo"
   * getPluginName("oh-my-opencode@2.4.3") // "oh-my-opencode"
   * getPluginName("@scope/pkg@1.0.0") // "@scope/pkg"
   * getPluginName("github:anthropics/claude-code/plugins/feature-dev") // "feature-dev"
   */
  export function getPluginName(plugin: string): string {
    if (plugin.startsWith("github:") || plugin.startsWith("https://github.com/")) {
      const parts = plugin.replace(/\.git$/, "").split("/")
   ...
   * 2. Local lobster.json
   * 3. Global plugin/ directory
   * 4. Global lobster.json
   *
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: lobster.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for lobster serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://github.com/HuskySteam/LOBSTER"),
      ...
      plugin_marketplaces: z.string().array().optional(),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://github.com/HuskySteam/LOBSTER"),
      provider: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "lobster.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "lobster.jsonc"))),
    )
  ...
  function globalConfigFile() {
    const candidates = ["lobster.jsonc", "lobster.json", "config.json"].map((file) =>
