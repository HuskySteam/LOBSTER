...
const managedConfigDir = process.env.LOBSTER_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "lobster.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "lobster.json") {
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "lobster.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads agents from .lobster/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lobster")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .lobster/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lobster")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .lobster/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lobster")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only LOBSTER_CONFIG_DIR", async () => {
  ...
  const prev = process.env.LOBSTER_CONFIG_DIR
  process.env.LOBSTER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.LOBSTER_CONFIG_DIR
    else process.env.LOBSTER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable LOBSTER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".lobster")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Bun.write(
        path.join(opencodeDir, "lobster.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/lobster")
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.lobster/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.lobster/plugin/oh-my-opencode.js")
  })
...
describe("LOBSTER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["LOBSTER_DISABLE_PROJECT_CONFIG"]
    process.env["LOBSTER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["LOBSTER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["LOBSTER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .lobster/ directories when flag is set", async () => {
    const originalEnv = process.env["LOBSTER_DISABLE_PROJECT_CONFIG"]
    process.env["LOBSTER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const opencodeDir = path.join(dir, ".lobster", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["LOBSTER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOBSTER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["LOBSTER_CONFIG_DIR"]
      process.env["LOBSTER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["LOBSTER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["LOBSTER_CONFIG_DIR"]
      } else {
        process.env["LOBSTER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("LOBSTER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["LOBSTER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOBSTER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["LOBSTER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["LOBSTER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from LOBSTER_CONFIG_DIR, not project
